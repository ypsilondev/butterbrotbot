...
public class BirthdayCommandOld implements GuildExecuteHandler {
    ...
    private String setBirthday(String asMention, Guild guild, String[] args, int start, GuildMessageReceivedEvent event) {
        ...
        StringBuilder saveValue = new StringBuilder(args[start]);
        for (int i = start + 1; i < args.length; i++) {
            saveValue.append(" ").append(args[i]);
        }

        saveValue = new StringBuilder(saveValue.toString().replaceAll(" ", ""));
        String[] dateComponents = saveValue.toString().split("\\.");
        if (dateComponents.length >= 2) {
            try {
                int day = Integer.parseInt(dateComponents[0]);
                int month = Integer.parseInt(dateComponents[1]);
                saveValue = new StringBuilder(day + "." + month + ".");
                if (dateComponents[2].toCharArray().length > 0) {
                    int year = Integer.parseInt(dateComponents[2]);
                    saveValue.append(year);
                }
                if (saveBirthday(saveName, saveValue.toString())) {
     ...
     * @param member
     */
    private void explainSyntaxError(Member member) {
